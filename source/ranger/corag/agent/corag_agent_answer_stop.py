...
class CoRagAgent:
    ...
    def __init__(
        ...
        self.subquery_times = []
        self.subanswer_times = []
        self.retrieve_times = []
    
    def print_average_times(self):
        """각 단계별 평균 시간을 출력합니다."""
        if self.subquery_times:
            print(f"평균 subquery 생성 시간: {sum(self.subquery_times) / len(self.subquery_times):.4f}초")
        if self.subanswer_times:
            print(f"평균 subanswer 생성 시간: {sum(self.subanswer_times) / len(self.subanswer_times):.4f}초")
        if self.retrieve_times:
            print(f"평균 문서 검색 시간: {sum(self.retrieve_times) / len(self.retrieve_times):.4f}초")
    ...
    def sample_path(
        ...
        while len(past_subqueries) < max_path_length and num_llm_calls < max_num_llm_calls:
            ...
            self.subquery_times.append(time.time() - subquery_start_time)
            ...
            if subanswer.lower() == answer.lower():
                break
    ...
    def _get_subanswer_and_doc_ids(
        ...
        self.retrieve_times.append(time.time() - retrieve_start_time)
        ...
        self._truncate_long_messages(messages, max_length=max_message_length)
        subanswer_start_time = time.time()
        subanswer: str = self.vllm_client.call_chat(messages=messages, temperature=0., max_tokens=128)
        self.subanswer_times.append(time.time() - subanswer_start_time)
