...
cut_points = [i*(len(corpus) - len(filtered_context_doc_ids)) // num_output_shards for i in range(1, num_output_shards+1)]
...
for shard_path in tqdm(shard_paths, desc="filter embeddings..."):
    ...
    for i in range(num):
        ...
        if current_idx not in filtered_context_doc_ids:
            shard_buffer.append(tensor[i].unsqueeze(0))
            num_selected += 1

        if shard_idx < len(cut_points) and num_selected >= cut_points[shard_idx]:
            save_path = os.path.join(output_dir, f"unused-shard-{shard_idx}.pt")
